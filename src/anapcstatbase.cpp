...
anapcstatbase::anapcstatbase(TString fileList) : fChain(0) 
{
  treeName = "T";
  ifstream indata;
...
anapcstatbase::anapcstatbase(TString inFileName, Int_t keyID) : fChain(0) 
{
  treeName = "T";
  if(keyID == 1){
    ifstream indata;
...
void anapcstatbase::tGraphInit(TGraph *gr[nChannels], TString grName, TString grTitle){
  Int_t i;
  TString grNameh;
  TString grTitleh;
...
void anapcstatbase::h1D1Init(TH1D *h1D1[nChannels],TString h1name, TString h1Title,
...
void anapcstatbase::h2D2Init(TH2D *h2D1[nChannels],TString h2name, TString h2Title,
...
void anapcstatbase::tProfInit(TProfile *tprof[nChannels],TString prname, TString prTitle,
		       Int_t Nbin, Float_t Vmin, Float_t Vmax){
  Int_t i;
  TString prnameh;
  TString prTitleh;
...
double anapcstatbase::getUnixTimeFromTime(double d_year, double d_month, double d_day, double d_hour, double d_min, double d_sec){
  //time_t timer;
  struct tm y2k = {0};
...
anapcstatbase::~anapcstatbase(){
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void anapcstatbase::Loop(TString histOut){
}

Int_t anapcstatbase::GetEntry(Long64_t entry){
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t anapcstatbase::LoadTree(Long64_t entry){
...
void anapcstatbase::Init(TTree *tree){
...
void anapcstatbase::Show(Long64_t entry){
...
Int_t anapcstatbase::Cut(Long64_t entry){
